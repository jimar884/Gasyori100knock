...
def rgb2hsv(img):
    ...
    min_channel = np.argmin(img, axis=2)
    ...
    result_img[..., 0][index] = 60 * (img[..., 1][index] - img[..., 2][index]) / (max_v[index] - min_v[index]) + 60
    index = np.where(min_channel == 1)     # min is green
    result_img[..., 0][index] = 60 * (img[..., 2][index] - img[..., 0][index]) / (max_v[index] - min_v[index]) + 300
    index = np.where(min_channel == 2)     # min is red
    result_img[..., 0][index] = 60 * (img[..., 0][index] - img[..., 1][index]) / (max_v[index] - min_v[index]) + 180
